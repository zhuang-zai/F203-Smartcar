...
void iic_delay()
{
    I2C_Delay_2us(); // 延时  或者使用 delay_us(us);
}
...
{
//    GPIO_InitTypeDef GPIO_Soft_I2C_Pin;
//
//    GPIO_Soft_I2C_Pin.Pin  = Soft_I2C_SCL_Pin|Soft_I2C_SDA_Pin;         //要初始化的I/O
//    GPIO_Soft_I2C_Pin.Mode = GPIO_OUT_PP;                               //要初始化的I/O 推挽输出模式
//    GPIO_Inilize(Soft_I2C_PORT,&GPIO_Soft_I2C_Pin);                     //初始化管脚配置
    P2_MODE_OUT_PP(Soft_I2C_SCL_Pin|Soft_I2C_SDA_Pin);      //这样初始化节省空间
}
