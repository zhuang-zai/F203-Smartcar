...
void DMA_M2M_Inilize(DMA_M2M_InitTypeDef *DMA)
{
	...
	if (DMA->DMA_SRC_Dir == M2M_ADDR_DEC)
		DMA_M2M_CFG |= 0x20; // 数据读完后地址自减
	else
		DMA_M2M_CFG &= ~0x20; // 数据读完后地址自减
	if (DMA->DMA_DEST_Dir == M2M_ADDR_DEC)
		DMA_M2M_CFG |= 0x10; // 数据写入后地址自减
	else
		DMA_M2M_CFG &= ~0x10; // 数据写入后地址自减

	if (DMA->DMA_Enable == ENABLE)
		DMA_M2M_CR |= 0x80; // 使能M2M DMA
	else
		DMA_M2M_CR &= ~0x80; // 禁止M2M DMA
}
...
void DMA_SPI_Inilize(DMA_SPI_InitTypeDef *DMA)
{
	...
	if (DMA->DMA_SS_Sel <= SPI_SS_P35)
		DMA_SPI_CFG2 = (DMA_SPI_CFG2 & 0xfc) | DMA->DMA_SS_Sel; // 自动控制SS脚选择

	if (DMA->DMA_Tx_Enable == ENABLE)
		DMA_SPI_CFG |= 0x40; // 使能SPI发送数据
	else
		DMA_SPI_CFG &= ~0x40; // 禁止SPI发送数据

	if (DMA->DMA_Rx_Enable == ENABLE)
		DMA_SPI_CFG |= 0x20; // 使能SPI接收数据
	else
		DMA_SPI_CFG &= ~0x20; // 禁止SPI接收数据

	if (DMA->DMA_AUTO_SS == ENABLE)
		DMA_SPI_CFG2 |= 0x04; // 使能SS自动控制
	else
		DMA_SPI_CFG2 &= ~0x04; // 禁止SS自动控制

	if (DMA->DMA_Enable == ENABLE)
		DMA_SPI_CR |= 0x80; // 使能SPI DMA
	else
		DMA_SPI_CR &= ~0x80; // 禁止SPI DMA
}
...
void DMA_UART_Inilize(u8 UARTx, DMA_UART_InitTypeDef *DMA)
{
	...
	if (UARTx == UART1)
	{
		...
		if (DMA->DMA_TX_Enable == ENABLE)
			DMA_UR1T_CR |= 0x80; // 使能UART1 TX DMA
		else
			DMA_UR1T_CR &= ~0x80; // 禁止UART1 TX DMA
		if (DMA->DMA_RX_Enable == ENABLE)
			DMA_UR1R_CR |= 0x80; // 使能UART1 RX DMA
		else
			DMA_UR1R_CR &= ~0x80; // 禁止UART1 RX DMA
	}
#endif
#ifdef UART2
	if (UARTx == UART2)
	...
	if (UARTx == UART3)
	{
		...
		if (DMA->DMA_TX_Enable == ENABLE)
			DMA_UR3T_CR |= 0x80; // 使能UART3 TX DMA
		else
			DMA_UR3T_CR &= ~0x80; // 禁止UART3 TX DMA
		if (DMA->DMA_RX_Enable == ENABLE)
			DMA_UR3R_CR |= 0x80; // 使能UART3 RX DMA
		else
			DMA_UR3R_CR &= ~0x80; // 禁止UART3 RX DMA
	}
#endif
#ifdef UART4
	if (UARTx == UART4)
	{
		...
		if (DMA->DMA_TX_Enable == ENABLE)
			DMA_UR4T_CR |= 0x80; // 使能UART4 TX DMA
		else
			DMA_UR4T_CR &= ~0x80; // 禁止UART4 TX DMA
		if (DMA->DMA_RX_Enable == ENABLE)
			DMA_UR4R_CR |= 0x80; // 使能UART4 RX DMA
		else
			DMA_UR4R_CR &= ~0x80; // 禁止UART4 RX DMA
	}
#endif
}
...
void DMA_LCM_Inilize(DMA_LCM_InitTypeDef *DMA)
{
	...
	if (DMA->DMA_Enable == ENABLE)
		DMA_LCM_CR |= 0x80; // 使能LCM DMA
	else
		DMA_LCM_CR &= ~0x80; // 禁止LCM DMA
}
...
void DMA_I2C_Inilize(DMA_I2C_InitTypeDef *DMA)
{
	...
	if (DMA->DMA_TX_Enable == ENABLE)
		DMA_I2CT_CR |= 0x80; // 使能I2CT DMA
	else
		DMA_I2CT_CR &= ~0x80; // 禁止I2CT DMA
	if (DMA->DMA_RX_Enable == ENABLE)
		DMA_I2CR_CR |= 0x80; // 使能I2CR DMA
	else
		DMA_I2CR_CR &= ~0x80; // 禁止I2CT DMA
}
