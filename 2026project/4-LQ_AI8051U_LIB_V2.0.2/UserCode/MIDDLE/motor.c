...
#define ONE_LOOP 1320000  //速度为250时  里程为1730000    为300时   里程为1800000   400时   1870000
int16 _encoder_L;
int16 _encoder_R;
int16 left_pwm;
int16 right_pwm;
...
/*pwm输出限幅*/
int16 max_forward_pwm = (int16)(3200 * 0.9375);  // 正转限制在约 85%
int16 max_reverse_pwm = (int16)(-3200 * 0.9375) ; // 反转限制在约 -40% (刹车力度够用就行)
double car_distance = 0.0f;    // 车辆行驶的累计里程
double global_distance = 0.0f; // ?? 新增全局里程：从发车开始全程累加，用于划定赛道区域
...
/**
 * @brief 电机速度控制函数：输入目标速度，读取当前速度，通过PID计算占空比，控制电机转速
...
//		}
	...
	//速度环占空比，分别用PID_LEFT和PID_RIGHT索引来实现左右轮控制
...
//		}
	...
	/*
	当1000占空比时对应编码器输出为1100，则认为左右轮最大转速为1100
	*/
...
void Fan_Smooth_Task(void)
{
    ...
    if (stop_flag == 1) 
    {
        target_fan_pwm = 1000; // 死机/待机：期望 1000
    }
    
    // 2. 利用 while(1) 的循环周期，进行非阻塞【现值追踪】
    fan_tick++;
    if(fan_tick >= 2) // 决定平滑的速度，数值越大变化越慢
    {
        fan_tick = 0;
