...
float tune_step = 0.01f;  // 默认步进值，可以通过左右键改变
/**
 * @brief 显示电磁传感器数据
 * 显示归一化后的五个电感值
...
{
		const PID_TypeDef *yaw_pid = PID_GetController(3); // 索引 3 是角速度环
...
//    
    ...
    if (selected_param == 0)
        sprintf(txt, "-> Kp: %4.2f     ", yaw_pid->Kp);
    else
        sprintf(txt, "   Kp: %4.2f     ", yaw_pid->Kp);
    LCD_ShowStr_16(0, 1, txt, u16_RED_IPS, u16_BLACK);
    ...
    if (selected_param == 2) // 为了逻辑清晰，我们用 0 代表 P，2 代表 D
        sprintf(txt, "-> Kd: %4.2f     ", yaw_pid->Kd);
    else
        sprintf(txt, "   Kd: %4.2f     ", yaw_pid->Kd);
    LCD_ShowStr_16(0, 2, txt, u16_RED_IPS, u16_BLACK);
		/*偏差*/
