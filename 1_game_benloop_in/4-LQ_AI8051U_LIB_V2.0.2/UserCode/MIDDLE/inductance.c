...
//    R2 = Get_Filtered_ADC(4); // R2
    ...
    //归一化后的ADC值
    ADC_values[0] = _adcNormalize(L1, 40, 2450);//L1
    ADC_values[1] = _adcNormalize(L2, 30, 2450);//L2
    ADC_values[2] = _adcNormalize(M, 0, 2500);//M
    ADC_values[3] = _adcNormalize(R1, 50, 2450);//R1
    ADC_values[4] = _adcNormalize(R2, 60, 2450);//R2
}
