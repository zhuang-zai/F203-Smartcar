...
// ?? 物理标定常数 (根据实际测试结果设定)
// ==========================================
#define STOP_EXTRA_DIST  32000  // ?? 跑完两圈冲线后，多跑约 0.5米 (1m ≈ 64000) 然后停车

// 假设你推车测得连续直角弯入口在 1500000，出口在 1600000 (请用 VOFA 实际测定)
#define RIGHT_ANGLE_ZONE_MIN  1630000   //速度为250时    1450000   为300时    1600000
#define RIGHT_ANGLE_ZONE_MAX  1750000   //速度为250时    1700000		为300时    1750000

// 速度档位定义
#define SPEED_BASE  200  // 直道和普通大弯的基础速度
#define SPEED_SLOW  200  // 连续直角弯的安全过弯速度
#define ROUNDABOUT_ZONE_MIN  500000  // 进入苯环预警区 (提前一点睁眼)
#define ROUNDABOUT_ZONE_MAX  900000000 // 离开苯环预警区 (过完环岛后彻底闭眼)
...
// 角度标定：1° ≈ 14.21
#define LOOP_EXIT_ANGLE  5300   // 环岛出口触发角度 (约 310°)
...
uint8 current_lap = 1;         // ?? 记录当前所在的圈数


/**
 * @brief 圈数统计与冲线停车任务
 * 监控 global_distance 的跳变来计算圈数，并在跑满两圈后延后一段距离自动停车
 */
void Lap_Control_Task(void)
{
    static double last_global_distance = 0.0f;
    
    // 如果系统处于急停或未发车状态，重置圈数和历史里程，等待下次发车
    if (stop_flag == 1)
    {
        current_lap = 1;
    ...
    // ?? 圈数统计：检测里程“清零跳变” (比如车子跨过起跑线时，里程从 1700000 突降到 0)
    // 设定 500000 为阈值，防止由于车子轻微倒车或数据波动导致的误判
    if (last_global_distance - global_distance > 500000.0f)
    {
        current_lap++; // 完成一圈，圈数加 1
    }
    last_global_distance = global_distance;

    // ?? 冲线停车逻辑：已经跑完了 2 圈 (意味着进入了第 3 圈)，且多跑了指定的冲线距离
    if (current_lap > 2 && global_distance > STOP_EXTRA_DIST)
    {
...
void Speed_Plan_Task(void)
{
    ...
    {
        chassis.target_speed = SPEED_SLOW; // 直接突变降速到 150
    }
    else
    {
        chassis.target_speed = SPEED_BASE; // 出弯后直接突变恢复到 300
    }
	}
}
/**
...
{
    chassis.target_speed = 200;       // 初始化目标速度为 300   max : 1100
    chassis.current_deviation = 0;  // 初始化当前偏差为 0
    chassis.left_speed = 0;         // 初始化左轮速度为 0
    chassis.right_speed = 0;        // 初始化右轮速度为 0
}
...
void Dynamic_Yaw_Test_Task(void)
{
    ...
    {
        case 0: target_yaw_rate = 4000;  break;  // 阶段0：正转 4000
        case 1: target_yaw_rate = -4000; break;  // 阶段1：反转 4000
        case 2: target_yaw_rate = 7000;  break;  // 阶段2：正转 7000
       case 3: target_yaw_rate = -7000; break;  // 阶段3：反转 7000
//        case 4: target_yaw_rate = 8500;  break;  // 阶段4：正转 8500
//        case 5: target_yaw_rate = 5500; break;  // 阶段5：反转 5500
...
void Roundabout_Detect_Task(void)
{
    ...
    switch(loop_state)
    {
        case LOOP_NORMAL:
                ...
                if (global_distance > ROUNDABOUT_ZONE_MAX || global_distance < ROUNDABOUT_ZONE_MIN)
            {
                // 不在苯环区域，强制归零所有相关计时器，并直接跳出，纯自主循迹！
                state_timer = 0;
                break; 
            }
    // ?? 识别左环
...
//            }
        ...
        case LOOP_TURN_IN:
            // 入环一定时间，或者侧边电感回落，认为已进环
             ...
             Clear_Car_Distance(); // 如果你在 IN_LOOP 里不用距离，清不清无所谓，保险起见清掉
            ...
            if(abs((int)car_angle) > LOOP_EXIT_EXTRA_ANGLE)
            {
                ...
                if(adc[2] < 80 && adc[0] < 60 && adc[3] < 60)
                {
                    ...
                    if(state_timer >= 1) // 20ms 防抖
                    {
                        ...
                        Clear_Car_Angle();
                        Clear_Car_Distance();
                        set_distance_calculation = 1; // 开启距离，给 NORMAL 里的冷却期测距！
                    }
                }
                else { state_timer = 0; }
            }
...
//      Dynamic_Yaw_Test_Task();
    ...
    if(outer_loop_timer >= 3) 
    {
        ...
        chassis.current_deviation = Calculate_Deviation();
				...
				// 当偏差绝对值小于 12 时 (车子行驶在直道中央区)，降低 P，让车头变“软”，防止高频画龙
        if (abs(chassis.current_deviation) <= 15) 
        {
            PID_SetKp(PID_DIRECTION, 300.0f); // 降低 P
        }
        // 当偏差绝对值大于 18 时 (车子进弯或者偏离赛道)，恢复原有高 P，让车头变“硬”，迅速拉回
        else PID_SetKp(PID_DIRECTION, 400.0f); // 恢复正常 P
       

        // 外环算出扭矩，更新 direction_output
...
//          direction_output = 0;
        ...
        if(k >= 0)
        {
            chassis.left_speed  = chassis.target_speed * (1 - 1.0 * k);  //k系数为1.0   速度300 - 200 时  1.0
            chassis.right_speed = chassis.target_speed * (1 + k * 0.4);// k系数为0.2		 速度300 - 200 时  0.4
        }
        else
        {
            k *= -1;
            chassis.left_speed  = chassis.target_speed * (1 + k * 0.4); //k系数为0.2
            chassis.right_speed = chassis.target_speed * (1 - k * 1.0);  //k系数为1.0
        }
